<template>
  <div>
    ...
    <!-- 添加参数的对话框 -->
    <el-dialog :title="'添加' + titleText" :visible.sync="addDialogVisible" width="30%" @closed="addDialogClosed">
      <!-- 添加参数的表单 -->
...
<!-- 修改参数的对话框 -->
<el-dialog :title="'修改' + titleText" :visible.sync="editDialogVisible" width="30%" @closed="editDialogClosed">
      <!-- 修改参数的表单 -->
  ...
  <span slot="footer" class="dialog-footer" slot-scope="scope">
    ...
    <el-button type="primary" @click="editParams(scope.row.attr_id)">确 定</el-button>
  </span>
</el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    editParams (id) {
      this.$refs.addFormRef.validate(async (valid) => {
        if (!valid) return
        const { data: res } = await this.$http.post(`categories/${this.cateId}/attributes/${id}`, this.addForm
        )
        if (res.meta.status !== 201) {
